...
import {decompressJpeg} from 'neuroglancer/sliceview/jpeg';
...
registerAsyncComputation(
    ...
    async function(
      ...
      let result = await decompressJpeg(
        data, parser.width, parser.height, parser.numComponents, 
        convertToGrayscale
      );

      if (!convertToGrayscale && numComponents > 1) {
        result = transposeArray2d(result, width * height, 3);
      }
